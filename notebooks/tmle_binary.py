...
ate_th_data = compute_ATE_theoretical_from_data(
    simulate_binary_data(10000, alpha=ALPHA, beta=BETA), BETA
)
...
models = [
    {"alpha": [0, 0.5, -0.5, 0], "beta": [-1, 2, 1, -1, 0]},
    {"alpha": [0.2, 1, -1, 0], "beta": [-1, 2, 1, -1, 0]},
    {"alpha": [0, 0.5, -0.5, 0], "beta": [1, 5, 0.1, -2, 0]},
]
for model in models:
...
print(
    "Difference between theoretical and TMLE ATE:",
    round(ate_th_data - TMLE_estimator(data)[0], 4),
)
...
estimators = [IPW_estimator, TMLE_estimator, AIPW_estimator]
...
models = {
    ...
    "small treatment effect": {"alpha": [0, 0.5, -0.5, 0], "beta": [1, 0.5, 1, -1, 0]},
...
diffs, stds = get_scores_for_models_and_estimators(
    patient_numbers, models, estimators, n_bootstraps=10
)
...
patient_numbers = [
    100,
    400,
]  # 1600, 3200, 6400, 12800]# 3200, 6400, 12800]
...
diffs, stds = get_scores_for_models_and_estimators(
    patient_numbers, models, estimators, n_bootstraps=1, estimator_args=estimator_args
)
