...
def simulate_data(n_subjects, max_time, censoring_rate_treated, censoring_rate_control):
    ...
    censoring_times_treated = np.random.exponential(
        scale=max_time / censoring_rate_treated, size=n_subjects
    )
    censoring_times_control = np.random.exponential(
        scale=max_time / censoring_rate_control, size=n_subjects
    )

    # Combine censoring times based on treatment
    censoring_times = np.where(
        treatment == 1, censoring_times_treated, censoring_times_control
    )
...
def run_simulation(
    n_simulations, n_subjects, max_time, censoring_rate_treated, censoring_rate_control
):
    results = []

    for _ in range(n_simulations):
        df = simulate_data(
            n_subjects, max_time, censoring_rate_treated, censoring_rate_control
        )
        # Fit Cox model
        cph = CoxPHFitter()
...
print(
    f"Proportion of significant p-values (< 0.05): {(results['p_value'] < 0.05).mean():.3f}"
)
...
kmf_treated.fit(
    df_example[df_example["treatment"] == 1]["time_to_event"],
    df_example[df_example["treatment"] == 1]["MACE_event_observed"],
    label="Treated",
)
kmf_control.fit(
    df_example[df_example["treatment"] == 0]["time_to_event"],
    df_example[df_example["treatment"] == 0]["MACE_event_observed"],
    label="Control",
)
