...
from ehr2vec.time2event_tmle.estimation import (
    estimate_ps,
    full_IPCW,
    transform_data_for_model_estimation,
    fit_failure_model,
    fit_censoring_model,
)
from ehr2vec.time2event_tmle.simulation_r import (
    simulate_data,
    theoretical_survival_function,
)
...
# %%
data = simulate_data(10000, censoring_kwargs={"scale": 15, "shape_multiplier": 0.5})
data = pd.DataFrame(data)
time_points = np.linspace(0.1, 40, 200)
...
survival_curve_untreated = theoretical_survival_function(
    time_points, data_treated["X"], 0
)
...
kmf.fit(
    data_treated["T_observed"], event_observed=data_treated["Y"], label="A=1 simulated"
)
kmf.plot(ax=ax, alpha=0.9, linestyle="--", color="skyblue")
ax.plot(time_points, survival_curve_treated, label="A=1 Theoretical", color="skyblue")

# Untreated
kmf.fit(
    data_untreated["T_observed"],
    event_observed=data_untreated["Y"],
    label="A=0 simulated",
)
kmf.plot(ax=ax, alpha=0.9, linestyle="--", color="orange")
...
for i, patient_num in enumerate([100, 1000, 10000]):
    data = simulate_data(
        patient_num, censoring_kwargs={"scale": 15, "shape_multiplier": 0.5}
    )
    data = pd.DataFrame(data)

    time_points = np.linspace(0.1, 30, 200)
    survival_curve_treated = theoretical_survival_function(time_points, data["X"], 1)
    ...
    # Treated
    ax[i].plot(
        time_points, survival_curve_treated, label="Theoretical", color="skyblue"
    )
...
# %%
data = pd.DataFrame(
    simulate_data(10000, censoring_kwargs={"scale": 15, "shape_multiplier": 0.5})
)
data = estimate_ps(data)
cls_data = transform_data_for_model_estimation(data)
failure_model = fit_failure_model(cls_data)
...
def estimate_survival_probability_for_all_t(
    ...
    if failure_model is None:
        ...
        cls_data.loc[:, failure_column] = failure_model.predict_proba(
            cls_data[["t", "A", "X"]]
        )[:, 1]
    cls_data.loc[:, f"S_{name}"] = (
        (1 - cls_data[failure_column]).groupby(cls_data["pid"]).transform("cumprod")
    )
    return cls_data
...
def compute_TMLE(
    data: pd.DataFrame,
    cls_data: pd.DataFrame,
    failure_model: object,
    censoring_model: object,
    treatment: int,
):
    cls_data = estimate_survival_probability_for_all_t(
        cls_data,
        "E",
        failure_model,
    )
        ...
        while (epsilon > 1e-2) and (j < 10):
            ...
            for i, patient in tqdm.tqdm(data.iterrows(), total=len(data)):  #
                ...
                for k in range(0, int(patient["T_observed"]) + 1):
                    ...
                    ht = compute_clever_covariate(
                        patient, cls_patient, t, k, A=treatment
                    )
            ...
            h_arr, N_arr, lambda_E_arr = (
                np.array(h_arr),
                np.array(N_arr),
                np.array(lambda_E_arr),
            )

            # get epsilon by running a logisitc regression on logit(N) = logit(lambda) + epsilon*h
            logit_lambda = logit(
                lambda_E_arr.clip(1e-10, 1 - 1e-10)
            )  # Avoid log(0) by clipping values
            # Prepare the data for logistic regression
            X = pd.DataFrame({"logit_lambda": logit_lambda, "h": h_arr})

            logistic_regression = LogisticRegression(
                fit_intercept=False, solver="lbfgs"
            )  # No intercept as we model logit(N) = logit(lambda) + epsilon*h
