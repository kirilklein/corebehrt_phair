...
class TestCalibrationPipeline(unittest.TestCase):
    def setUp(self):
        ...
        self.predictions_df.to_csv(os.path.join(self.finetune_folder, "predictions_and_targets.csv"), index=False)
        ...
        for fold in range(1, self.n_splits + 1):
            ...
            train_pids = torch.tensor([1, 2, 3, 4]) if fold == 1 else torch.tensor([5, 6, 7, 8])
            val_pids = torch.tensor([5, 6, 7, 8]) if fold == 1 else torch.tensor([1, 2, 3, 4])
