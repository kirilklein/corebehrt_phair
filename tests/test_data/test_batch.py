...
class TestBatches(unittest.TestCase):
    ...
    @patch(
        "ehr2vec.data.batch.load_assigned_pids",
        return_value={
            "pretrain": [str(i) for i in range(10)],
            "finetune": [str(i) for i in range(90, 100)],
        },
    )
    def setUp(self, *args):
        self.cfg = ConfigMock()
...
class TestBatchTokenize(unittest.TestCase):
    ...
    @patch(
        "ehr2vec.data.batch.BatchTokenize.load_and_filter_batch",
        return_value=(
            {
                "concept": [
                    ["BG_GENDER_MALE", "Diagnosis1", "Medication1", "Diagnosis2"],
                    ["BG_GENDER_FEMALE", "Diagnosis3", "Medication1"],
                ],
    ...
    @patch(
        ...
        side_effect=[
            (
                {
                    "concept": [["BG_GENDER_MALE", "Diagnosis1", "Medication1"]],
                    "segment": [[0, 1, 2]],
                },
                ["1"],
            ),
            (
                {"concept": [["BG_GENDER_MALE", "Diagnosis1"]], "segment": [[0, 1]]},
                ["2"],
            ),
    ...
    def test_tokenize(self, tokenize, save, save_tokenized_data):
        splits = {
            "pretrain": Split(mode="pretrain", pids=["1"]),
            "finetune": Split(mode="finetune", pids=["2"]),
            "test": Split(mode="test", pids=["3"]),
        }
        ...
        self.assertEqual(
            self.batch_tokenize.save_tokenized_data.call_args_list[0],
            call(*expected_results, save_dir=None),
        )
        ...
        self.assertEqual(
            self.batch_tokenize.save_tokenized_data.call_args_list[1],
            call(*expected_results2, save_dir=None),
        )
        ...
        self.assertEqual(
            self.batch_tokenize.save_tokenized_data.call_args_list[2],
            call(*expected_results3, save_dir=None),
        )
