...
from ehr2vec.data.creators import (
    AbsposCreator,
    AgeCreator,
    BackgroundCreator,
    DeathCreator,
...
class TestBaseCreator(unittest.TestCase):
        ...
        self.patients_info = pd.DataFrame(
            {
                ...
                "DATE_OF_DEATH": pd.to_datetime(
                    ["2020-01-02", "2021-03-20", "2022-05-08"]
                ),
    ...
    def test_abspos_creator(self):
        ...
        self.assertEqual(
            result.ABSPOS.tolist(),
            (
                (self.concepts.TIMESTAMP - origin_point).dt.total_seconds() / 60 / 60
            ).tolist(),
        )
    ...
    def test_background_creator(self):
        ...
        for _, patient in result.groupby("PID"):
            self.assertEqual(
                patient.CONCEPT.str.startswith("BG_").sum(), len(self.cfg.background)
            )

    def test_death_date_creator(self):
        # Ensure SEGMENT is added to the concepts DataFrame
        segment_creator = SegmentCreator(self.cfg)
        concepts_with_segment = segment_creator.create(
            self.concepts, self.patients_info
        )
        ...
        expected_ages = (
            (
                (
                    self.patients_info["DATE_OF_DEATH"]
                    - self.patients_info["BIRTHDATE"]
                ).dt.days
                / 365.25
            )
            .round(2)
        ...
        abspos_at_death = Utilities.get_abspos_from_origin_point(
            self.patients_info["DATE_OF_DEATH"], self.cfg.abspos
        )
        self.assertEqual(death_entries["ABSPOS"].tolist(), abspos_at_death.tolist())

        for pid in self.patients_info["PID"]:
            self.assertEqual(
                death_entries[death_entries["PID"] == pid]["SEGMENT"].iloc[0],
                concepts_with_segment[concepts_with_segment["PID"] == pid][
                    "SEGMENT"
                ].iloc[-1],
            )
