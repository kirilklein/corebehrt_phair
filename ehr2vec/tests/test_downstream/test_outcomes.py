...
class MockUtilities:
    ...
    def get_abspos_from_origin_point(
        ...
        if isinstance(timestamps, pd.Series):
            ...
            return [
                (timestamp - origin_point_dt).total_seconds() / 60 / 60
...
class TestOutcomeHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.outcomes_df = pd.DataFrame(
            {"PID": ["P1", "P2", "P3", "P4"], "TIMESTAMP": [None, 100, 200, None]}
        )

        self.exposures_df = pd.DataFrame(
            {"PID": ["P1", "P2", "P3", "P4"], "TIMESTAMP": [0, 50, 150, 50]}
        )
    ...
    def test_assign_time2event(self):
        ...
        self.assertEqual(
            result_data.times2event,
            [30.0, 50.0, 50.0, 60.0],
            "Time to event calculation is incorrect.",
        )
    ...
    def test_get_death_abspos(self):
        ...
        np.testing.assert_array_equal(
            result, expected_death_abspos, "Death positions are incorrectly computed."
        )
    ...
    def test_compute_end_of_time_abspos(self):
        ...
        expected_end_time = MockUtilities.get_abspos_from_origin_point(
            [end_of_time_dt], OutcomeHandler.ORIGIN_POINT
        )[0]
        result = self.handler.compute_end_of_time_abspos()
        self.assertEqual(
            result[0], expected_end_time, "End of time computation is incorrect."
        )
