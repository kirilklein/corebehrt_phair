...
def finetune_fold(
    ...
    if cfg.model.get("scale_with_frequency", False):
        concept_frequency = compute_concept_frequency(
            train_data.features, train_data.vocabulary
        )
    ...
    perturbation_model = PerturbationModel(
        finetuned_model, cfg.model, concept_frequency
    )
    ...
    assert len(train_data.vocabulary) == len(
        perturbation_model.get_sigmas_weights()
    ), f"Vocabulary size {len(train_data.vocabulary)} does not match sigmas size {len(perturbation_model.get_sigmas_weights())}"
