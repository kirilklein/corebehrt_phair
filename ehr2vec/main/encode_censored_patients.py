...
def _get_output_path_name(dataset, cfg):
    ...
    if cfg.path.exposure:
        ...
        window = (
            int(abs(cfg.outcome.n_hours_censoring / 24))
            if days
            else abs(cfg.outcome.n_hours_censoring)
        )
...
def main_encode():
    ...
    for i, outcome in enumerate(cfg.outcomes):
        ...
        logger.info(
            f"Censoring {cfg.outcome.n_hours} hours after {cfg.outcome.censor_type}"
        )
        ...
        if i == 0:
            ...
            shutil.copy(
                join("outputs", "tmp", "info.log"), join(cfg.output_dir, "info.log")
            )
        ...
        logger.info(
            f"Store in directory with name: {_get_output_path_name(dataset, cfg)}"
        )
        ...
        if cfg.env == "azure":
            save_to_blobstore(
                cfg.paths.output_path, join(BLOBSTORE, censored_patients_path)
            )
    if cfg.env == "azure":
        mount_context.stop()
    logger.info("Done")
