...
def compute_fold(
    ...
    with torch.no_grad():
        for i, batch in enumerate(get_tqdm(dataloader)):
            ...
            concepts = (
                batch["concept"].numpy().reshape(-1, 1, batch["concept"].shape[1])
            )
            explainer = shap.PermutationExplainer(
                wrapped_model, masker=masker, batch_size=shap_batch_size
            )
            ...
            all_shap_values = insert_concept_shap_values(
                all_shap_values, concepts, shap_values
            )
            log_most_important_features(
                all_shap_values, data.vocabulary, num_features=20
...
def cv_loop_predefined_splits(
    data: Data, predefined_splits_dir: str, finetune_folder: str, fi_folder: str, logger
) -> int:
...
if __name__ == "__main__":
    data, mount_context, cfg, run, logger, fi_folder, azure_context = (
    ...
    shap_values = cv_loop_predefined_splits(
        data,
