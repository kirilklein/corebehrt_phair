...
def save_outcomes(data, folder, split):
    ...
    outcomes = torch.tensor(
        [1 if pd.notna(outcome) else 0 for outcome in data.outcomes]
    ).float()
    torch.save(outcomes, join(folder, f"{split}_outcomes_binary.pt"))


def process_and_save(
    data: Data, func: callable, name: str, split: str, folder: str
) -> tuple:
...
def save_stats(
    finetune_folder: str, train_val_data: Data, test_data: Data = None
) -> None:
    """Save basic stats"""
    ...
    positive_stats.to_csv(
        join(finetune_folder, "patient_stats_positives.csv"), index=True
    )
...
def _limit_train_patients(indices_or_pids: list) -> list:
    if "number_of_train_patients" in cfg.data:
        if len(indices_or_pids) >= cfg.data.number_of_train_patients:
            ...
            logger.info(
                f"Number of train patients is limited to {cfg.data.number_of_train_patients}"
            )
        else:
            raise ValueError(
                f"Number of train patients is {len(indices_or_pids)}, but should be at least {cfg.data.number_of_train_patients}"
            )
...
if __name__ == "__main__":
    ...
    if "abspos" not in data.features:
        raise ValueError(
            "No absolute positions found in data, needed to calculate stats. Check remove features tag in config and make sure BehrtAdapter is not activated as it removes abspos."
        )
    logger.info("Splitting data")
    ...
    if "predefined_splits" in cfg.paths:
        ...
        train_val_data = data.select_data_subset_by_pids(
            train_val_pids, mode="train_val"
        )
    else:
        test_data, train_val_indices = split_into_test_data_and_train_val_indices(
            cfg, data
        )
        train_val_data = data.select_data_subset_by_indices(
