...
CONFIG_NAME = "finetune_evaluate_example.yaml"
BLOBSTORE = "CINF"
...
def compute_and_save_scores_mean_std(
    n_splits: int, test_folder: str, mode="test", logger=None
) -> None:
...
def main():
    ...
    fold_dirs = [
        fold_folder
        for fold_folder in os.listdir(finetune_folder)
    ...
    if not cfg.data.get("preprocess", False):
        ...
        if "predefined_pids" in cfg.paths:
            ...
            if len(test_pids) != len(set(test_pids)):
                logger.warn(
                    f"Test pids contain duplicates. Test pids len {len(test_pids)}, unique pids {len(set(test_pids))}."
                )
