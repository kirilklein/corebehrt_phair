...
def finetune_fold(
    ...
    train_dataset = Time2EventDataset(
        train_data.features, train_data.outcomes, train_data.times2event
    )
    val_dataset = Time2EventDataset(
        val_data.features, val_data.outcomes, val_data.times2event
    )
    test_dataset = (
        Time2EventDataset(test_data.features, test_data.outcomes, test_data.times2event)
        if len(test_data) > 0
        else None
    )
...
def cv_loop_predefined_splits(
    data: Data, predefined_splits_dir: str, test_data: Data, run=None
) -> int:
