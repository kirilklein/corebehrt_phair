...
class Censorer:
    def __init__(
        ...
        self.censor_diag_separately = self.censor_diagnoses_at_end_of_visit or (
            self.n_hours_diag_censoring != self.n_hours
        )
    ...
    def censor(self, features: dict, index_dates: list) -> dict:
        ...
        if not self.background_length:
            raise ValueError(
                "Background length is not computed. Please call compute_background_length first."
            )
        censored_features = {key: [] for key in features}
        censor_loop = tqdm(
            iter_patients(features),
            desc="Censoring",
    ...
    def _generate_censor_flags(
        ...
        if self.censor_diag_separately:
            if self.censor_diagnoses_at_end_of_visit:
                diag_censor_flags = self._generate_sep_diag_censor_flags_end_of_visit(
    ...
    def _generate_sep_diag_censor_flags(
        ...
        abs_pos_flags = [
            True if abspos <= (index_timestamp + self.n_hours_diag_censoring) else False
            for abspos in patient["abspos"]
        ]
        return self._combine_lists_with_and(diagnoses_sep_flags, abs_pos_flags)

    def _generate_sep_diag_censor_flags_end_of_visit(
        self, patient: Dict[str, list], index_timestamp: float
    ) -> List[bool]:
        """
        ...
        """
        last_segment = self._get_last_segment_before_timestamp(
            patient["segment"], patient["abspos"], index_timestamp
        )
        last_index_to_include = self._return_last_index_for_element(
            patient["segment"], last_segment
        )
        diagnoses_sep_flags = self.get_combined_diag_sep_flags(patient["concept"])
        return [
    ...
    def get_combined_diag_sep_flags(
        self, concepts: List[Union[int, str]]
    ) -> List[bool]:
        """
    ...
    @staticmethod
    def _get_last_segment_before_timestamp(
        segments: List[int], abspos: List[float], index_timestamp: float
    ) -> int:
    ...
    def _identify_background(
        self, concepts: List[Union[int, str]], tokenized_flag: bool
    ) -> List[bool]:
        """
