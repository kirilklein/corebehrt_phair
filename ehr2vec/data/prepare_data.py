...
class DatasetPreparer:
    ...
    def prepare_finetune_data(
        ...
        if predefined_pids:
            ...
            if self.cfg.outcome.get("control_code", None) is not None:
                ...
                data.exposures = [1 if pid in data.exposed_patients else 0 for pid in data.pids]
            data.check_lengths()
        ...
        if not predefined_pids:
            ...
            outcomehandler = OutcomeHandler(
                ...
                drop_pids_w_outcome_pre_followup=self.cfg.outcome.get(
        ...
        data = self.utils.process_data(
            data,
            self.patient_filter.exclude_dead_patients,
            log_positive_patients_num=True,
        )
    ...
    def _prepare_mlm_features(self) -> Data:
        ...
        # 3. Exclude short sequences
        data = self.utils.process_data(
            data, self.patient_filter.exclude_short_sequences
        )
        if not predefined_pids:
