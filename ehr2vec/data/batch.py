...
class Batches:
    ...
    def __init__(self, cfg: Config, pids: List[List[str]]):
        ...
        if self.predefined_splits_dir is None:
            ...
            assigned_pids = self.filter_assigned_pids(
                assigned_pids, self.flattened_pids
            )
            for split, split_assigned_pids in assigned_pids.items():
                logger.info(
                    f"Number of assigned pids in split {split}: {len(split_assigned_pids)}"
                )
            ...
            self.flattened_pids = [
                pid for pid in self.flattened_pids if pid not in pids_to_exclude_set
            ]
    ...
    def split_batches(self) -> Dict[str, Split]:
        ...
        if self.predefined_splits_dir is not None:
            logger.warn(
                f"Loading predefined splits from {self.predefined_splits_dir}. Ignores all other settings related to splits."
            )
        ...
        total_pids = len(
            self.flattened_pids
        )  # total number of pids, excluding the assigned pids

        for pids in self.assigned_pids.values():
            total_pids += len(
                pids
            )  # include the assigned pids in the total number of pids to calculate the remaining ratios

        self.update_split_ratios(total_pids)
        self.shuffle_pids()

        # calculate the number of batches for each set
        finetune_end, pretrain_end = self.calculate_split_indices(
            len(self.flattened_pids)
        )
    ...
    def get_predefined_splits(self) -> Dict[str, Split]:
        ...
        split_files = {
            file.split("_")[1].split(".")[0]: file
            for file in os.listdir(self.predefined_splits_dir)
            if file.startswith("pids_")
        }
        assert (
            len(split_files) > 0
        ), f"No predefined splits found in {self.predefined_splits_dir}"
        ...
        for mode, file in split_files.items():
            ...
            if not set(pids).issubset(set(self.flattened_pids)):
                ...
                logger.warning(
                    f"Predefined pids in mode {mode} contain pids that are not in the dataset. {diff}"
                )
                pids = list(set(pids).intersection(set(self.flattened_pids)))
            assert len(pids) == len(
                set(pids)
            ), f"Predefined pids for split {mode} contain duplicates."
            all_predefined_pids_set.update(set(pids))
            splits[mode] = Split(pids=pids, mode=mode)
        assert all_predefined_pids_set.issubset(
            set(self.flattened_pids)
        ), f"Predefined pids are not a subset of all pids."
    ...
    def update_split_ratios(self, total_length: int) -> None:
        ...
        allocated_counts = {
            split: len(pids) for split, pids in self.assigned_pids.items()
        }
        ...
        for split, ratio in self.split_ratios.items():
            ...
            if allocated_counts.get(split, 0) > max_allowed_pids:
                raise ValueError(
                    f"Cannot satisfy split ratios for split '{split}'. Allocated count exceeds the ratio limit {int(ratio * total_length)}."
                )
        ...
        for split, ratio in self.split_ratios.items():
            ...
            remaining_ratios[split] = (
                target_ratio / remaining_length if remaining_length > 0 else 0
            )
    ...
    @staticmethod
    def filter_assigned_pids(
        assigned_pids: Dict[str, List[str]], pids: List[str]
    ) -> Dict[str, List[str]]:
...
class BatchTokenize:
    ...
    def save_vocabulary(self) -> None:
        ...
        self.tokenizer.save_vocab(
            join(self.cfg.output_dir, self.tokenized_dir_name, "vocabulary.pt")
        )
    ...
    def tokenize_split(self, split: Split, features_dir: str) -> None:
        ...
        assert split_pids_set.issubset(
            set(self.pid2fileid.keys())
        ), f"Split pids ({len(split_pids_set)}) is not a subset of pid2fileid keys ({len(self.pid2fileid.keys())})"
        pid2fileid = {
            pid: file_id
            for pid, file_id in self.pid2fileid.items()
            if pid in split_pids_set
        }
        fileid2pid = self.invert_dictionary(pid2fileid)
        # select only file ids
        for file_id, selected_pids_in_file in tqdm(
            fileid2pid.items(),
            desc=f"Tokenizing {split.mode} batches",
            file=TqdmToLogger(logger),
        ):
            filtered_features, filtered_pids = self.load_and_filter_batch(
                file_id, selected_pids_in_file, features_dir
            )
        ...
        assert set(split.pids) == set(
            pids
        ), f"Split pids ({len(split.pids)}) and pids ({len(pids)}) do not match"
        encoded, pids = Utilities.select_and_reorder_feats_and_pids(
            encoded, pids, split.pids
        )

        assert len(pids) == len(
            encoded["concept"]
        ), f"Length of pids ({len(pids)}) does not match length of encoded ({len(encoded['concept'])})"

        return encoded, pids

    @staticmethod
    def load_and_filter_batch(
        file_id: str, selected_pids_in_file: List[str], features_dir: str
