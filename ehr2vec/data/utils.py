...
class Utilities:
    ...
    def process_data(
        ...
        data: Data,
        func: callable,
        log_positive_patients_num: bool = False,
        args_for_func: Dict = {},
    ) -> Dict:
    ...
    @staticmethod
    def get_relative_timestamps_in_hours(
    ...
    def get_token_to_index_map(vocabulary: dict) -> Tuple[dict]:
        ...
        new_vocab = {
            k: token2index[v] for k, v in vocabulary.items() if v in token2index
        }
    ...
    def log_pos_patients_num(data: Data) -> None:
        ...
        if num_positive_patiens < MIN_POSITIVES[data.mode]:
            raise ValueError(
                f"Number of positive patients is less than {MIN_POSITIVES[data.mode]}: {num_positive_patiens}"
            )
    ...
    @staticmethod
    def split_train_val(
        features: Dict[str, List], pids: List, val_ratio: float = 0.2
    ...
    def iter_patients(data) -> Generator[dict, dict, dict]:
        ...
        for i in range(len(data.features["concept"])):
            yield {key: values[i] for key, values in data.features.items()}, {
                key: values[i] for key, values in data.outcomes.items()
            }
    ...
    def _generate_censor_flags(
        absolute_positions: List[float], event_timestamp: float
    ) -> List[bool]:
        """Generate flags indicating which items to censor."""
        return [position - event_timestamp <= 0 for position in absolute_positions]
    ...
    def calculate_ages_at_censor_date(data: Data) -> List[int]:
        ...
        for abspos, age, censor_date in zip(
            data.features["abspos"], data.features["age"], data.index_dates
        ):
    ...
    @staticmethod
    def get_first_non_special_token_index(
        concepts: List[int], special_tokens: set
    ) -> List[int]:
        """
        Get the first non-special token for each patient.
        """
        return next(
            (i for i, token in enumerate(concepts) if token not in special_tokens), -1
        )
    ...
    def calculate_trajectory_lengths(data: Data) -> List[float]:
        ...
        special_tokens = set(
            [
                data.vocabulary[token]
                for token in data.vocabulary
                if token.startswith(("[", "BG_"))
            ]
        )
        for abspos, concept, censor_date in zip(
            data.features["abspos"], data.features["age"], data.index_dates
        ):
            first_concept_index = Utilities.get_first_non_special_token_index(
                concept, special_tokens
            )
    ...
    def calculate_sequence_lengths(data: Data) -> List[int]:
        ...
        for concept in data.features["concept"]:
            non_special_tokens = [
                token for token in concept if token not in special_tokens
            ]
