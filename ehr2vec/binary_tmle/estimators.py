...
def cross_validated_outcome_estimation(
    data: pd.DataFrame, covariates: list, model: object, cv_folds: int
):
    ...
    for train_index, val_index in skf.split(data, data["A"]):
        ...
        data = estimate_all_outcomes(
            data, val_data, covariates, outcome_model, val_index
        )
...
def estimate_counterfactual_outcome(
    data: pd.DataFrame, covariates: list, fitted_model: object, treatment: int
):
