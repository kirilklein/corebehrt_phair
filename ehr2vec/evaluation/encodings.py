...
class Forwarder(EHRTrainer):
        ...
        with torch.no_grad():
            for i, batch in enumerate(
                tqdm(
                    self.dataloader,
                    desc="Batch Forward",
                    file=TqdmToLogger(self.logger) if self.logger else None,
                )
            ):
                self.to_device(batch)
                batch_pids = self.dataset.pids[
    ...
    def obtain_filter_mask(
        self, input: torch.Tensor, selected_concept_ints: torch.Tensor
    ) -> torch.Tensor:
        # Expand dimensions to make tensors broadcastable
        input = input.unsqueeze(1)  # Add a new dimension to tensor1
        selected_concept_ints = selected_concept_ints.unsqueeze(
            0
        )  # Add a new dimension to tensor2
        # Check for equality
    ...
    def produce_patient_trajectory_embeddings(self, start_code_id: int = 4) -> dict:
        ...
        data = {
            k: []
            for k in self.dataset.features.keys()
            if k in ["concept", "age", "abspos"]
        }
        ...
        with torch.no_grad():
            for i, batch in enumerate(
                ...
                pat_counts = [
                    j for j in range(i * self.batch_size, (i + 1) * self.batch_size)
                ]
                if i == len(self.dataloader):
                    pat_counts = [
                        ...
                        for j in range(
                            i * self.batch_size,
                            i * self.batch_size + len(batch["concept"]),
                        )
                    ]
                for length in tqdm(
                    range(start_code_id, batch["concept"].shape[1]), desc="Truncation"
                ):
                    trunc_batch = self.censor(batch, length)
                    self.to_device(trunc_batch)
                    output = self.forward_pass(trunc_batch)
                    mask = (
                        trunc_batch["attention_mask"][:, -1]
                        .detach()
                        .numpy()
                        .flatten()
                        .astype(bool)
                    ...
                    for feat in data:  # concept, age, abspos
                        feat_arr = (
                            (trunc_batch[feat][:, -1]).detach().numpy().flatten()[mask]
                        )
    ...
    def produce_concept_embeddings(self) -> dict:
        ...
        with torch.no_grad():
            for i, batch in enumerate(
                ...
                output = self.forward_pass(batch)
                hidden_states = (
                    torch.flatten(output.hidden_states[-1], end_dim=1).detach().numpy()
                )
    ...
    def get_relevant_ids(self, cfg, vocabulary: dict) -> torch.tensor:
        return torch.tensor(
            [
                ...
                if any(k.startswith(char) for char in cfg.filter_concepts)
            ],
            dtype=int,
        )
...
class EHRTester:
    def __init__(
        ...
        self._initialize_basic_attributes(
            model, test_dataset, metrics, cfg, run, accumulate_logits, mode
        )
    ...
    def _initialize_basic_attributes(
        self, model, test_dataset, metrics, cfg, run, accumulate_logits, mode
    ):
        self.device = (
