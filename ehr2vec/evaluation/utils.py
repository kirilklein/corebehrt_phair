...
def validate_outcomes(all_outcomes, cfg):
    for outcome in cfg.outcomes:
        ...
        if cfg.outcome.type:
            assert (
                cfg.outcome.type in all_outcomes
            ), f"Outcome {cfg.outcome.type} not found in outcomes."
        if cfg.outcome.get("censor_type", False):
            assert (
                cfg.outcome.censor_type in all_outcomes
...
def evaluate_predictions(
    y_val: np.ndarray, pred_probas: np.ndarray, metrics: list, threshold: float = 0.5
):
    results = {}
...
def check_data_for_overlap(
    ...
    if test_data is not None:
        ...
        assert (
            len(train_pids.intersection(test_pids)) == 0
        ), "Train and test data overlap"
...
def check_predefined_pids(data: Data, cfg) -> None:
    if "predefined_splits" in cfg.paths:
        ...
        if not set(all_predefined_pids).issubset(set(data.pids)):
            ...
            raise ValueError(
                f"Pids in the predefined splits must be a subset of data.pids. There are {difference} pids in the data that are not in the predefined splits"
            )
...
def save_data(data: Data, folder: str) -> None:
    ...
    if len(data) > 0:
        ...
        if data.exposed_patients is not None:
            torch.save(
                data.exposed_patients, join(folder, f"{data.mode}_exposed_pids.pt")
            )
        if data.exposures is not None:
            torch.save(data.exposures, join(folder, f"{data.mode}_exposures.pt"))
    else:
