...
class BaseHDF5Writer:
    def __init__(
        self, output_path, encodings_dataset_name="X", targets_dataset_name="y"
    ):
    ...
    def initialize(self, hidden_dim):
        ...
        with h5py.File(self.output_path, "a") as f:
            ...
            if self.targets_dataset_name not in f:
                f.create_dataset(
                    self.targets_dataset_name, shape=(0,), maxshape=(None,), dtype=int
                )
...
class PatientHDF5Writer(BaseHDF5Writer):
    ...
    def __init__(
        self,
        output_path,
        encodings_dataset_name="X",
        pid_dataset_name="pids",
    ...
    def initialize(self, hidden_dim):
        ...
        with h5py.File(self.output_path, "a") as f:
            if self.pid_dataset_name not in f:
                ...
                f.create_dataset(
                    self.pid_dataset_name, shape=(0,), maxshape=(None,), dtype=dt
                )
...
class PatientHDF5Reader(BaseHDF5Reader):
    ...
    def __init__(
        self, input_path, encodings_ds_name="X", pid_ds_name="pids", target_ds_name="y"
    ):
...
def load_input_target(
    ...
    if acc_method == "mean":
        file_name = (
            f"{set_}_embedding_censor_{censor_outcome}_12h_{time}_{acc_method}.pt"
        )
...
def get_train_test_for_embeddings(
    data_path: str,
    censor_outcome: str,
    target: str,
    post: bool,
    ...
    if test_set_only:
        sets = ["test"]
    for set_ in sets:
        X_temp, y_temp = load_input_target(
            data_path, set_, censor_outcome, target, post, acc_method, layers
        )
        ...
        if filter_covid:
            ...
            covid_mask = torch.tensor(
                [1 if isinstance(x, float) else 0 for x in covid_outcome]
            ).bool()
